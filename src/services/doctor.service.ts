...
class DoctorService {
  // Doctor registration
  public async signUp(body: { email: string; password: string; doctor_name: string; specialization: string }): Promise<any> {
    const doctor = await DoctorModel.findOne({ email: body.email });
    if (!doctor) {
      throw new Error('Doctor not registered by admin');
    }
    body.password = await bcrypt.hash(body.password, 10);
    const updatedDoctor = await DoctorModel.updateOne({ email: body.email }, body, { new: true });
    const spArray = await specializationModel.findOne({$where:'this.specializations.length>0'});
    if(!spArray){
    ...
    if(spArray.specializations.find(specialization => specialization===body.specialization)===undefined){
      spArray.specializations.push(body.specialization);
      spArray.save();
    }
    return updatedDoctor;
  ...
  public async login(body: { email: string; password: string }): Promise<{ token: string; refreshToken: string }> {
    ...
    const token = jwt.sign({ id: doctor._id }, process.env.JWT_DOCTOR as string, { expiresIn: '1h' });
    const refreshToken = jwt.sign({ id: doctor._id }, process.env.JWT_DOCTOR as string, { expiresIn: '7d' });
  ...
  public async getAllAppointments(
    ...
    const appointments = await appointmentsModel.find({ specialization }).skip(skip).limit(limit);
...
public async updateAilmentStatus(id: string, ailment_status: boolean): Promise<any> {
  const updatedAppointment = await appointmentsModel.findByIdAndUpdate(id, { ailment_status }, { new: true });
...
public deletePatientById = async (patientId: string): Promise<void> => {
  ...
  public async generateBill(id,{ prescription, invoice }): Promise<Ibill> {
    ...
    const bill = await billModel.create({appointment_id:appointment.id,doctor_id: appointment.doctor_id,patient,prescription,invoice});
    return bill;
  }
  ...
  public async forgotPassword(email: string): Promise<void> {
    ...
    const token = jwt.sign({ id: doctor._id }, process.env.JWT_RESET_DOCTOR as string, { expiresIn: '1h' });
  ...
  public async refreshToken(doctorId: string): Promise<string> {
    ...
    const payload = jwt.verify(doctor.refreshToken, process.env.JWT_DOCTOR as string) as { id: string };
    const newToken = jwt.sign({ id: payload.id }, process.env.JWT_DOCTOR as string, { expiresIn: '1h' });
    return newToken;
  }
}
